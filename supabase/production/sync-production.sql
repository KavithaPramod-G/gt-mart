-- Idempotent sync for staging or production (safe to re-run)
-- Category UI, storage buckets, live categories seed
...
on conflict (id) do update set label = excluded.label, emoji = excluded.emoji,
  tint = excluded.tint, accent = excluded.accent, blurb = excluded.blurb,
